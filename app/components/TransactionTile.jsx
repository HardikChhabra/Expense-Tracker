...
const TransactionTile = ({ desc, type, date, amount, deleteFn, editFn }) => {
  return (
    <li className="border grid grid-cols-12 gap-4 py-2 px-4">
      <div className="col-span-5 text-sm font-bold text-gray-900 text-wrap">
        {desc}
      </div>
      ...
      {amount < 0 ? (
        <div className="col-span-2 text-sm font-semibold text-red-500">
          -${-amount}
        </div>
      ) : (
        <div className="col-span-2 text-sm font-semibold text-green-500">
          ${amount}
        </div>
      )}
